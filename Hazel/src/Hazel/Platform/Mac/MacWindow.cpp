...
MacWindow::MacWindow(const WindowProperties &properties) { Init(properties); }
...
void MacWindow::Init(const WindowProperties &properties) {
  ...
  HZ_CORE_INFO("Creating window {0} ({1}, {2})", properties.Title,
  ...
  glfwSetMouseButtonCallback(
      m_Window, [](GLFWwindow *window, int key, int action, int mods) {
        (void)action;
        (void)mods;
        WindowData &data = *(WindowData *)glfwGetWindowUserPointer(window);
